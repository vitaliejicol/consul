...
func (a *Agent) listenHTTP() ([]apiServer, error) {
	...
	start := func(proto string, addrs []net.Addr) error {
		...
		for _, l := range listeners {
			...
			if proto == "https" {
				if err := setupHTTPServer(httpServer, connLimitFn, a.config.HTTPSHandshakeTimeout); err != nil {
					return err
				}
			} else {
...
// setupHTTPServer adds HTTP/2 support, ConnState, and a connection handshake timeout
// to the http.Server.
func setupHTTPServer(server *http.Server, connState func(net.Conn, http.ConnState), timeout time.Duration) error {
...
func (a *Agent) ShutdownEndpoints() {
	...
	err := a.apiServers.WaitForShutdown()
	if err != nil {
